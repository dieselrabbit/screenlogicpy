...
import socket
import struct
from . msgutil import getMessageString, makeMessage, takeMessage
from . const import me, code
from . decode_response.config import decode_pool_config_response
from . decode_response.status import decode_pool_status_response
...
def request_gateway(gateway_socket):
    ...
    if (rcvcode != code.VERSION_ANSWER):
        sys.stderr.write("WARNING: {}: rcvCode({}) != {}.\n".format(me, rcvcode, code.VERSION_ANSWER))
        return False
        #sys.exit(10)
